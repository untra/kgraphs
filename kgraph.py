...
from networkx.classes.multigraph import MultiDiGraph
from networkx.exception import NetworkXError
__author__ = """\n""".join(['Aric Hagberg (hagberg@lanl.gov)',
                            'Pieter Swart (swart@lanl.gov)',
                            'Dan Schult(dschult@colgate.edu)',
                            'Samuel Volin(samuel.volin@colorado.edu)'])

class KGraph(MultiDiGraph):
    """A multicolored directed graph class that can store multiedges.

    Multiedges are multiple edges between two nodes.  Each edge
    can hold optional data or attributes.

    A KGraph holds directed edges.  Self loops are allowed.

    KGraphs have unique properties under certain circumstances. These methods are provided.
